...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "HShare is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "HShare is downloading blocks...");
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (pindexBest->nHeight >= Params().LastPOWBlock(pindexBest->nHeight))
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "HShare is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "HShare is downloading blocks...");
