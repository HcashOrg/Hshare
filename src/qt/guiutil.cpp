...
{
    // Convert hshare:// to hshare:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("hshare://"))
    {
        uri.replace(0, 12, "hshare:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "HShare.lnk";
}
...
{
    return GetAutostartDir() / "hshare.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("HShare-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  hshare-qt [" + tr("command-line options") + "]                     " + "\n";
