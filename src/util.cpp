...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "HShare";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "hshare.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "hshared.pid"));
